...
def main(
    ...
    http: bool = typer.Option(
        False, "--http", help="Start Streamable HTTP server instead of stdio"
    ),
    ...
    auth_token: str = typer.Option(
        None, "--auth-token", envvar=ENV_AUTH_TOKEN, help="Bearer token for HTTP"
    ),
    wine_prefix: str = typer.Option(
        None, "--wine-prefix", envvar=ENV_WINEPREFIX, help="Wine prefix"
    ),
):
    if wine_prefix:
    ...
    if http:
        ...
        typer.echo(
            f"Starting HTTP server on {host}:{port} auth={'enabled' if auth_token else 'disabled'}",
            err=True,
        )
