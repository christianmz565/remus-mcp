...
from .jet.schema import REQUIRED_FIELDS, WRITABLE_TYPES
...
def validate_create(type_name: str, data: dict[str, Any], db_path: str) -> list[dict[str, str]]:
    ...
    if type_name not in WRITABLE_TYPES and type_name != "trace":
        errors.append(
            {"field": "type", "code": "READ_ONLY_TYPE", "message": f"Type {type_name} is read-only"}
        )
    # Required fields
            ...
            errors.append(
                {
                    "field": f,
                    "code": "REQUIRED",
                    "message": f"Field {f} is required for {type_name}",
                }
            )
    # Name length
                ...
                errors.append(
                    {
                        "field": "document",
                        "code": "FK_NOT_FOUND",
                        "message": f"document oid {doc_oid} not found",
                    }
                )
                ...
                errors.append(
                    {
                        "field": "parent",
                        "code": "FK_NOT_FOUND",
                        "message": f"parent oid {parent_oid} not found",
                    }
                )
        except Exception as e:
