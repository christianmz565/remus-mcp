...
def create_server(session_manager: SessionManager) -> Server:
    ...
    async def list_tools():
        return [
            Tool(
                name="open_project",
                description="Open a .rem project file",
                inputSchema={
                    "type": "object",
            ...
            Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        "project_id": {"type": "string"},
                        "source_oid": {"type": "integer"},
                        "target_oid": {"type": "integer"},
                        "checked": {"type": "boolean"},
                    },
                    "required": ["project_id", "source_oid", "target_oid"],
                },
            ),
            Tool(
                name="trace_remove",
            ...
            Tool(
                name="get_traces",
                description="Get traces for oid",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "project_id": {"type": "string"},
                    ...
                    "required": ["project_id", "oid"],
                },
            ),
            Tool(
                name="trace_matrix",
                description="Trace matrix",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "project_id": {"type": "string"},
                        "source_type": {"type": "string", "enum": REM_TYPE_VALUES},
                        "target_type": {"type": "string", "enum": REM_TYPE_VALUES},
                        "document": {"type": ["integer", "null"]},
                    },
                    "required": ["project_id", "source_type", "target_type"],
                },
            ),
            Tool(
            ...
            Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        ...
                        "filter_ids": {"type": ["array", "null"], "items": {"type": "integer"}},
                    },
                    "required": ["project_id"],
                },
            ),
            ...
            Tool(
                name="render_html",
                description="Render HTML/PDF via Wine msxml3",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "project_id": {"type": "string"},
                        ...
                        "lang": {"type": "string", "enum": ["en", "es", "de"]},
                        "output": {"type": "string", "enum": ["html", "pdf"]},
                    },
            ...
            Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        "template": {
                            "type": "string",
                            "enum": ["english", "spanish", "german", "empty"],
                        },
                        "target_path": {"type": "string"},
                        "name": {"type": "string"},
                    },
                    "required": ["template", "target_path", "name"],
                },
            ),
            Tool(
                name="project_clone",
        ...
        try:
            if name == "open_project":
                ...
                return [
                    TextContent(
                        type="text",
                        text=json.dumps({"project_id": pid, "path": arguments["path"]}, indent=2),
                    )
                ]
            elif name == "close_project":
                session_manager.close_project(arguments["project_id"])
                return [
                    TextContent(type="text", text=json.dumps({"closed": arguments["project_id"]}))
                ]
            elif name == "list_projects":
                return [
                    TextContent(
                        type="text",
                        text=json.dumps(session_manager.list_projects(), indent=2, default=str),
                    )
                ]
            elif name == "rem_list":
                res = crud_tools.rem_list(
                    session_manager,
                    ...
                    arguments.get("order_by", "order"),
                )
                return [TextContent(type="text", text=json.dumps(res, indent=2, default=str))]
            elif name == "rem_get":
                res = crud_tools.rem_get(
                    session_manager, arguments["project_id"], arguments["type"], arguments["oid"]
                )
                return [TextContent(type="text", text=json.dumps(res, indent=2, default=str))]
            elif name == "rem_create":
                res = crud_tools.rem_create(
                    session_manager, arguments["project_id"], arguments["type"], arguments["data"]
                )
                return [TextContent(type="text", text=json.dumps(res, indent=2, default=str))]
            elif name == "rem_update":
                res = crud_tools.rem_update(
                    session_manager,
                    arguments["project_id"],
                    arguments["type"],
                    arguments["oid"],
                ...
                res = trace_tools.trace_add(
                    session_manager,
                    arguments["project_id"],
                    arguments["source_oid"],
                    arguments["target_oid"],
                    arguments.get("checked", False),
                )
                return [TextContent(type="text", text=json.dumps(res, indent=2, default=str))]
            elif name == "trace_remove":
                res = trace_tools.trace_remove(
                    session_manager,
                    arguments["project_id"],
                    arguments["source_oid"],
                    arguments["target_oid"],
                ...
                res = xml_tools.export_xml(
                    session_manager,
                    arguments["project_id"],
                    arguments.get("document"),
                    arguments.get("filter_type"),
                    arguments.get("filter_ids"),
                )
                # Truncate xml for display if large > 50k? Return path + snippet
                out = {
                    "path": res["path"],
                    "dtd_errors": res["dtd_errors"],
                    "xml_preview": res["xml"][:2000],
                    "xml_length": len(res["xml"]),
                }
                return [TextContent(type="text", text=json.dumps(out, indent=2))]
            elif name == "import_xml":
                res = xml_tools.import_xml(
                    session_manager,
                    arguments["project_id"],
                    arguments.get("xml"),
                    ...
                    arguments.get("confirm_replace", False),
                    arguments.get("on_missing_ref", "error"),
                )
                return [TextContent(type="text", text=json.dumps(res, indent=2, default=str))]
            elif name == "render_html":
                res = gen_tools.render_html(
                    session_manager,
                    arguments["project_id"],
                    arguments["document"],
                    arguments.get("lang", "en"),
                    arguments.get("output", "html"),
                )
                out = {
                    "path": res["path"],
                    "warnings": res["warnings"],
                    "html_preview": res["html"][:5000],
                    "html_length": len(res["html"]),
                }
                return [TextContent(type="text", text=json.dumps(out, indent=2))]
            elif name == "project_create":
                res = sys_tools.project_create(
                    session_manager,
                    arguments["template"],
                ...
                res = sys_tools.project_clone(
                    session_manager, arguments["project_id"], arguments["target_path"]
                )
                return [TextContent(type="text", text=json.dumps(res, indent=2, default=str))]
            elif name == "undo_last":
                ...
                res = sys_tools.get_change_log(
                    session_manager, arguments["project_id"], arguments.get("limit", 50)
                )
                return [TextContent(type="text", text=json.dumps(res, indent=2, default=str))]
            else:
            ...
            if any(
                ...
                return [
                    TextContent(
                        type="text", text=json.dumps({"error": msg, "code": code}, indent=2)
                    )
                ]
            # For file not found etc.
            return [
                TextContent(
                    type="text", text=json.dumps({"error": msg, "code": err_code}, indent=2)
                )
            ]
    ...
    async def get_prompt(name: str, arguments: dict[str, Any] | None):
        ...
        return GetPromptResult(
            messages=[
                PromptMessage(
                    role=m["role"], content=TextContent(type="text", text=m["content"]["text"])
                )
                for m in msgs
            ]
        )
